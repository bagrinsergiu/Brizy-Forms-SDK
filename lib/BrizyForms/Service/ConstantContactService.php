...
class ConstantContactService extends Service
{
        ...
        foreach ($fieldMap->toArray() as $fieldLink) {
            if ($fieldLink->getTarget() == ServiceConstant::AUTO_GENERATE_FIELD) {
                ...
                if ($key_exist === false) {
                    ...
                    $newCustomField = $this->client->api("list/field_add", $payload);
                }
    ...
    protected function internalCreateMember(
        ...
        if (!(int)$contact_sync->success) {
            $this->logger->error(
                json_encode($contact_sync),
                ['service' => ServiceFactory::ACTIVECAMPAIGN, 'method' => 'internalCreateMember']
            );
    ...
    {
        $lists = $this->client->api("list/list?ids=all");
    ...
    {
        $fields = $this->client->api("list/field_view?ids=all");
        $clearFields = [];
    ...
    {
        $account = $this->client->api("account/view");
